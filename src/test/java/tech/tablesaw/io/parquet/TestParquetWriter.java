...
class TestParquetWriter {
  ...
  public static void assertTableEquals(final Table expected, final Table actual, final String header) {
	    assertEquals(actual.rowCount(), expected.rowCount(), header + " tables should have same number of rows");
	    assertEquals(
	        actual.columnCount(), expected.columnCount(), header + " tables should have same number of columns");
	...
	void testReadWriteAllTypeDict() throws IOException {
		final Table orig = new TablesawParquetReader().read(
				TablesawParquetReadOptions.builder(APACHE_ALL_TYPES_DICT).build());
		new TablesawParquetWriter().write(orig, TablesawParquetWriteOptions.builder(OUTPUT_FILE).build());
		final Table dest = new TablesawParquetReader().read(
				TablesawParquetReadOptions.builder(OUTPUT_FILE).build());
	...
	void testReadWriteAllTypePlain() throws IOException {
		final Table orig = new TablesawParquetReader().read(
				TablesawParquetReadOptions.builder(APACHE_ALL_TYPES_PLAIN).build());
		new TablesawParquetWriter().write(orig, TablesawParquetWriteOptions.builder(OUTPUT_FILE).build());
		final Table dest = new TablesawParquetReader().read(
				TablesawParquetReadOptions.builder(OUTPUT_FILE).build());
	...
	void testReadWriteAllTypeSnappy() throws IOException {
		final Table orig = new TablesawParquetReader().read(
				TablesawParquetReadOptions.builder(APACHE_ALL_TYPES_SNAPPY).build());
		new TablesawParquetWriter().write(orig, TablesawParquetWriteOptions.builder(OUTPUT_FILE).build());
		final Table dest = new TablesawParquetReader().read(
				TablesawParquetReadOptions.builder(OUTPUT_FILE).build());
