...
class TestParquetReader {
	...
	void testTableNameFromFile() throws IOException {
		final Table table = PARQUET_READER.read(
				TablesawParquetReadOptions.builder(new File(PARQUET_TESTING_FOLDER, APACHE_ALL_TYPES_DICT)).build());
	...
	void testAllTypesDict() throws IOException {
		validateAllTypesDefault(APACHE_ALL_TYPES_DICT, TablesawParquetReadOptions
				.builder(PARQUET_TESTING_FOLDER + APACHE_ALL_TYPES_DICT).minimizeColumnSizes(), 2);
	}

	@Test
	void testAllTypesPlain() throws IOException {
		validateAllTypesDefault(APACHE_ALL_TYPES_PLAIN, TablesawParquetReadOptions
				.builder(PARQUET_TESTING_FOLDER + APACHE_ALL_TYPES_PLAIN).minimizeColumnSizes(), 8);
	}

	@Test
	void testAllTypesSnappy() throws IOException {
		validateAllTypesDefault(APACHE_ALL_TYPES_SNAPPY, TablesawParquetReadOptions
				.builder(PARQUET_TESTING_FOLDER + APACHE_ALL_TYPES_SNAPPY).minimizeColumnSizes(), 2);
	}
	...
	void testBinarySkip() throws IOException {
		final Table table = PARQUET_READER
				.read(TablesawParquetReadOptions.builder(PARQUET_TESTING_FOLDER + APACHE_BINARY)
						.withUnnanotatedBinaryAs(UnnanotatedBinaryAs.SKIP).build());
	...
	void testFixedLengthDecimalLegacy() throws IOException {
		final Table table = PARQUET_READER.read(TablesawParquetReadOptions
				.builder(PARQUET_TESTING_FOLDER + APACHE_FIXED_LENGTH_DECIMAL_LEGACY).build());
