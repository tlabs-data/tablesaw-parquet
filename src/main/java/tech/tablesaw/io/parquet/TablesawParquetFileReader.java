...
public class TablesawParquetFileReader implements DataReader<TablesawParquetReadOptions> {
	...
	private static final TablesawParquetFileReader INSTANCE = new TablesawParquetFileReader();
	...
	public Table read(final TablesawParquetReadOptions options) throws IOException {
		final Path path = new Path(options.source().file().getAbsolutePath());
		...
		try (final ParquetFileReader reader = ParquetFileReader.open(hif, opts)) {
			...
			while (true) {
				...
				for (int i = 0; i < rows; i++) {
					...
					row = nextRow(row, table);
					fillRow(row, group, options);
				}
			}
		}
	}

	private Row nextRow(final Row row, final Table table) {
		if(row == null || table.columnCount() == 0) return table.appendRow();
